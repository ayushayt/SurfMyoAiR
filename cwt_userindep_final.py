...
from tensorflow.keras.layers import concatenate, Lambda, Conv2D, MaxPooling2D, GlobalAveragePooling2D,LSTM,Bidirectional,Concatenate
...
def process_EMG(folder,duration,width,interp_type):

    X = np.zeros((260,64,64,5))
    Y = np.zeros((260,))
    testctr = 0
    ...
    for trial in range(1,6):
        for rep in range (1,3):
            for num in range(65,91):
                emg_sig = np.load('EMG_Data/'+folder+'/Segmented/EMG_'+chr(num)+'_'+str(trial)+'_'+str(rep)+'.npy')
...
for fold_no in range(0,5):
    ...
    #print('Fold number : ' + str(fold_no))
    x = np.arange(fold_no*10,(fold_no+1)*10)

    for subj_id in range(0,50):
        if subj_id not in x:
            [X,Y] = process_EMG(folderarr[subj_id],8000,60,'cubic')
            X_train[int(trainrefctr*260):int((trainrefctr+1)*260),:,:,:] = X
            Y_train[int(trainrefctr*260):int((trainrefctr+1)*260)] = Y
            trainrefctr = trainrefctr + 1
        else:
            [X,Y] = process_EMG(folderarr[subj_id],8000,60,'cubic')
            X_test[int(testrefctr*260):int((testrefctr+1)*260),:,:,:] = X
            Y_test[int(testrefctr*260):int((testrefctr+1)*260)] = Y
            testrefctr = testrefctr + 1
            
    np.save('cwt/X_train_cwt_userindep_fold_'+str(fold_no)+'.npy',X_train)
    np.save('cwt/Y_train_cwt_userindep_fold_'+str(fold_no)+'.npy',Y_train)
    np.save('cwt/X_test_cwt_userindep_fold_'+str(fold_no)+'.npy',X_test)
    np.save('cwt/Y_test_cwt_userindep_fold_'+str(fold_no)+'.npy',Y_test)
